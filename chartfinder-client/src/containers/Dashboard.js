  ...
  render() {
    return (
      <div>
        ...
        {this.props.user_charts ? (
          this.props.user_charts.map((chart) => (
            <div key={uuid()}>
              <button
                key={uuid()}
                onClick={() =>
                  this.props.submitDate(
                    chart.date.split("-").reverse().join("-")
                  )
                }
              >
                {chart.date} - Daily Chart
              </button>{" "}
              <DeleteChart chart_id={chart.id} birthday={false} />
            </div>
          ))
        ) : (
        ...
        {this.props.user_birthdays ? (
          this.props.user_birthdays.map((chart) => (
            <div key={uuid()}>
              <button
                key={uuid()}
                onClick={() =>
                  this.props.submitBirthday(
                    chart.date.split("-").reverse().join("-")
                  )
                }
              >
                {chart.date} - Birthday Chart
              </button>{" "}
              <DeleteChart chart_id={chart.id} birthday={true} />
            </div>
          ))
        ) : (
