    ...
    return (
      <div>
        ...
        {this.props.user_charts ? this.props.user_charts.map((chart) => (<div key={uuid()}><button key={uuid()} onClick={()=>this.props.submitDate(chart.date.split("-").reverse().join("-"))}>
            {chart.date} - Daily Chart</button> <DeleteChart chart_id={chart.id} birthday={false}/></div>)) : <p>no charts to show</p>}
            ...
            {chart.date} - Birthday Chart</button> <DeleteChart chart_id={chart.id} birthday={true}/></div>)) : <p>no birthdays to show</p>}
      </div>
    );
  }
}
